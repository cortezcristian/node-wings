...
;(function($){
	...
	cmdshell = {
		...
		cmdsPath: './cmds/cmd.',
		localCmds : [ 'cat','clear','eval','help','history','load','ls','psmgr'],
		...
		runCmd: function(cmd){
			try{
				...
				if(found == 0 && cmdshell.allowExecution){
					cmdshell.debuger('[CMD] Executing Console OS command: ' + cmd);
					$.ajax({
						type: "GET",
						url: cmdshell.urlExecution,
						data: {cmd:cmd},
						processData: true,
						async:false,
	...
	$.fn.jconsole = function(obj){
		...
		*/
		...
		return this.each(function() {
			...
			curIns.append('<div class="'+o.cmdOut+'"></div>'+o.symbol+'<input class="'+o.cmdIn+'" type="text" value="" />');
			o.cmdIn = '.'+o.cmdIn;
			o.cmdOut = '.'+o.cmdOut;
			//curIns.find(o.cmdOut).append(o.symbol);
			curIns.find(o.cmdIn).keydown(function(e) {
				...
				if(resultCmd){//Show the result of the command
					var cmdOut = $(o.cmdOut), lines = resultCmd.split("\n\n");
					
					cmdOut.append('<div>'+o.symbol+' '+valueCmd+o.sep+'</div>');
					
					for(var nLine in  lines){
						cmdOut.find('div').last().append('<pre style="inline">'+ lines[nLine].replace(/</g,"&lt;").replace(/>/g,"&gt;")+'</pre>'+o.sep);
					}
